    ...
    describe("dynamic numbers", () => {
        it("string should't collide with number", () => {
            for (let i = 0; i <= 1024; i++) {
                ...
                assert.equal(switchStructureCheck(bytes, { offset: 0 }), false, `string "${i}" misunderstood as switch structure`);
            }
        });
        ...
        it("uint8 shouldn't collide", () => {
            ...
            assert.equal(switchStructureCheck(bytes, { offset: 0 }), false, `misunderstood as switch structure`);
        })
