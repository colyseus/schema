    ...
    while (it.offset < totalBytes) {
      ...
      if (type == "ref") {
        ...
        for (i in 0...length) {
          ...
          if (
            it.offset >= bytes.length ||
            bytes.get(it.offset) == SPEC.END_OF_STRUCTURE
          ) {
              break;
          }
          ...
          var newKey = (hasMapIndex)
              ? previousKeys[decoder.number(bytes, it)]
              : decoder.string(bytes, it);

          var item: Dynamic;
          var isNew = (!hasIndexChange && !cast(valueRef.items, Map<String, Dynamic>).exists(newKey)) || (hasIndexChange && previousKey == "" && hasMapIndex);
